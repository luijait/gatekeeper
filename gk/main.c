...
dump:
	...
	case GK_REQUEST:
		ret = snprintf(state_msg, sizeof(state_msg),
			"gk: log the flow state %s[state: GK_REQUEST (%hhu), flow_hash_value: 0x%x, expire_at: 0x%"PRIx64", last_packet_seen_at: 0x%"PRIx64", last_priority: %hhu, allowance: %hhu, grantor_ip: %s] at %s() with lcore %u",
			s_in_use, fe->state, fe->flow_hash_val, fe->expire_at,
			fe->u.request.last_packet_seen_at,
			fe->u.request.last_priority, fe->u.request.allowance,
			grantor_ip, __func__, rte_lcore_id());
		break;
	case GK_GRANTED:
		ret = snprintf(state_msg, sizeof(state_msg),
			"gk: log the flow state %s[state: GK_GRANTED (%hhu), flow_hash_value: 0x%x, expire_at: 0x%"PRIx64", budget_renew_at: 0x%"PRIx64", tx_rate_kib_cycle: %u, budget_byte: %"PRIu64", send_next_renewal_at: 0x%"PRIx64", renewal_step_cycle: 0x%"PRIx64", grantor_ip: %s] at %s() with lcore %u",
			s_in_use, fe->state, fe->flow_hash_val, fe->expire_at,
	...
	case GK_DECLINED:
		ret = snprintf(state_msg, sizeof(state_msg),
			"gk: log the flow state %s[state: GK_DECLINED (%hhu), flow_hash_value: 0x%x, expire_at: 0x%"PRIx64", grantor_ip: %s] at %s() with lcore %u",
			s_in_use, fe->state, fe->flow_hash_val, fe->expire_at,
			grantor_ip, __func__, rte_lcore_id());
		break;
	case GK_BPF: {
		...
		ret = snprintf(state_msg, sizeof(state_msg),
			"gk: log the flow state %s[state: GK_BPF (%hhu), flow_hash_value: 0x%x, expire_at: 0x%"PRIx64", program_index=%u, cookie=%016"PRIx64", %016"PRIx64", %016"PRIx64", %016"PRIx64", %016"PRIx64", %016"PRIx64", %016"PRIx64", %016"PRIx64", grantor_ip: %s] at %s() with lcore %u",
			s_in_use, fe->state, fe->flow_hash_val, fe->expire_at,
			fe->program_index,
	...
	default:
		ret = snprintf(state_msg, sizeof(state_msg),
			"gk: log the unknown flow %s[state: %hhu, flow_hash_value: 0x%x, expire_at: 0x%"PRIx64", grantor_ip: %s] at %s() with lcore %u",
			s_in_use, fe->state, fe->flow_hash_val, fe->expire_at,
			grantor_ip, __func__, rte_lcore_id());
		break;
	}
 ...
 *
 * 3. The code should minimally really on the internals of
...
gk_del_flow_entry_at_pos(struct gk_instance *instance, uint32_t entry_idx)
{
	 ...
	 * to identify any corruption; including flow entries that invalid
